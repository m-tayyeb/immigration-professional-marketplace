...
export function CaseAppointmentAgreement({ caseId, caseStatus, role, userId, clientId, professionalUserId, assessmentPaid, appointment }: {
  caseId: string;
  caseStatus: string;
  role: UserRole;
  userId: string;
  ...
  const openAssessment = caseStatus === "AWAITING_ASSESSMENT_REVIEW" || caseStatus === "AWAITING_ASSESSMENT_PAYMENT";
  ...
  {professional && openAssessment && (!appointment || ["PROPOSED", "CHANGE_REQUESTED"].includes(appointment.status)) && <div className="mt-6 grid gap-5 lg:grid-cols-2"><AppointmentForm caseId={caseId} action={proposeAppointment} title={appointment ? "Send revised proposal" : "Send CaseWiser proposal"} submitLabel="Send proposal"/><AppointmentForm caseId={caseId} action={bookConfirmedAppointment} title="Book confirmed appointment" submitLabel="Record confirmed appointment" direct/></div>}
