...
import { hasDocumentStudioEntitlement } from "../../lib/document-studio";
...
export default async function DocumentStudioPage() {
  ...
  const [payments, cases] = await Promise.all([session.user.role === "CLIENT" ? prisma.paymentRequest.findMany({ where: { case: { clientId: session.user.id } }, select: { stage: true, status: true } }) : Promise.resolve([]), prisma.case.findMany({ where: caseWhere, include: { service: true }, orderBy: { updatedAt: "desc" } })]);
  ...
  return <DocumentStudio role={session.user.role} cases={cases.map((item) => ({ id: item.id, service: item.service.name }))}/>;
}
